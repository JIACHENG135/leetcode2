# -*- coding: utf-8 -*-
"""
Created on Sun Nov 18 11:06:25 2018
        ...
        #        str_in_len = len(str_in)
            if str_in[0] in str_all:
                ...
                if type(first_ind)!=int or first_ind !=0:
                    ...
                    i = first_ind
                    while i < len(str_all)-1:            
                        while str_all[i] == str_in[0] and i<len(str_all)-1:
                            if len(str_in) == 1:
                                if i not in ans:
                                    ans.append(i)
                                break
...
#                                if word[:len(word)-iii] in can_temp_1:
                                ...
                                if iii == 0 and index_need:
#                                    print(matrix[0:i+1,0:j],word[:len(word)-iii])
...
#                                if word[:len(word)-iii] in can_temp_2:
                                ...
                                if iii == 0 and index_need:
#                                        print(can_temp_1,word[:len(word)-ii])
...
#        print(matrix.shape[0],matrix.shape[1])
    ...
    #            print(i+1,j+1,(i+1)*(j+1))
                if (i+1)*(j+1)>len(word):
                    ...
                    [can_temp_1,can_index_1] = spiralOrder(list(matrix[0:i+1,0:j]))
                    [can_temp_2,can_index_2] = spiralOrder(list(matrix_t[0:j+1,0:i]))
                    for ii in range(len(word)):
    #                    print(i,j,ii,word,word[:len(word)-ii],can_temp_1)
    #                    print(word[:len(word)-ii] in can_temp_1)
    #                    print(i,j,matrix.shape,can_temp_1,word)
    
                        if word[:len(word)-ii] in can_temp_1:
#                            print(matrix[0:i+1,0:j],word[:len(word)-ii])

                            index_need = find_index(can_temp_1,word[:len(word)-ii],word[:len(word)-ii])
    #                        print(index_need)
                            if ii == 0:
                                return True
                            else:
    ...
    #                                print(index_m)
                                    [new_board_1,new_board_2,new_board_3] = gene_new_board(matrix,index_m)
#                                    print(i,j,new_board_1,new_board_2,new_board_3,word[len(word)-ii:])
                                    if not exist_head(new_board_1, word[len(word)-ii:]):
                                        pass
                                    else:
                                        return exist_head(new_board_1, word[len(word)-ii:])
#                                    if new_board_2:   
#                                        print(2)
                                    if not exist_head(new_board_2, word[len(word)-ii:]):
                                        pass
                                    else:
                                        return exist_head(new_board_2, word[len(word)-ii:])
                                    if not exist_head(new_board_3, word[len(word)-ii:]):
                                        pass
                                    else:
                                        return exist_head(new_board_3, word[len(word)-ii:])
                        if word[:len(word)-ii] in can_temp_2:
                            index_need = find_index(can_temp_2,word[:len(word)-ii],word[:len(word)-ii])
